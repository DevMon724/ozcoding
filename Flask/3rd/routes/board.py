...
class BoadsList(MethodView):
    ...
    def post(self):
        ...
        return jsonify({'msg' : 'success add data'}), 201
...
class BoardResource(MethodView):
    ...
    def put(self, board_id):
        ...
        db.session.commit()
        return jsonify({'msg' : 'Successfully updated Database'})

    def delete(self, board_id):
        board = Board.query.get_or_404(board_id)
        db.session.delete(board)
        db.session.commit()
        return jsonify({'msg' : 'Delete Data Complete'})